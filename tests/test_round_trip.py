...
def test_round_trip(
    tmp_path, input_file, method, randomize_caps, randomize_costs, num_commodities
):
    """Verifies that saved and loaded file content matches the generated multicommodity data."""
    base_name = os.path.basename(input_file)
    output_path = tmp_path / f"rt_{base_name}_u{method}_k{num_commodities}_rcap{randomize_caps}_rcost{randomize_costs}.mcfmin"
    ...
    assert loaded_mc_data.method == method
    ...
    expected_seed = 0 if (method == 1 and not randomize_caps and not randomize_costs) else seed
